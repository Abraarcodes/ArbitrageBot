...
for ech in fees:
    ...
    fees[ech] = {
        'base': (0 if markets['BTC/USDT']['feeSide'] != 'base' else markets['BTC/USDT']['taker']) if
        list(markets['BTC/USDT'].keys()).count('feeSide') != 0 else 0,
        'quote': (markets['BTC/USDT']['taker'] if markets['BTC/USDT']['feeSide'] != 'base' else 0) if
        list(markets['BTC/USDT'].keys()).count('feeSide') != 0 else markets['BTC/USDT']['taker']
    }
    ...
    for echange in echanges_str:
        try:
            ...
            if bal > (float(10) / float(ex[echange].fetch_ticker(pair_to_sell)['last'])):
                ...
                print(f"Not enough {pair_to_sell[:len(pair_to_sell) - 5]} on {echange}.")
            i += 1
    ...
    for echange in exlist:
        try:
            ...
            if bal > (float(10) / float(ex[echange].fetch_ticker(pair_to_sell)['last'])):
                ...
                print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Not enough {pair_to_sell[:len(pair_to_sell) - 5]} on {echange}.")
            i += 1
        except Exception as e:
            print(
                f'{Style.DIM}[{time.strftime("%H:%M:%S", time.gmtime(time.time()))}]{Style.RESET_ALL} Problem on {echange}. Error:    {e}')
...
while ordersFilled != len(echanges):
    ...
    try:
        print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Fetching the global average price for {currentPair}...")
        for n in echanges:
        ...
        print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Average {currentPair} price in {endPair}: {average_first_buy_price}")
    ...
    for n in echanges:
        time.sleep(0.7)
        print(f'{Style.DIM}{get_time()}{Style.RESET_ALL} Buy limit order of {round(total_crypto / len(echanges), 3)} {currentPair.split("/")[0]} at {average_first_buy_price} sent to {echanges_str[i]}.')
        i += 1

    print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} All orders sent.")

    already_filled = []
    for zz in range(len(echanges)):
        time.sleep(2.1)
        print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} {echanges_str[zz]} order filled.")
        ordersFilled += 1
        already_filled.append(exc)

print(f"{Style.DIM}{get_time()}{Style.RESET_ALL} Starting program with parameters: {[n for n in sys.argv]}")
prec_time = '0000000'
min_ask_price = 0
total_change_usd = 0
...
async def symbol_loop(exchange, symbol):
    ...
    while time.time() <= timeout:
        try:
            ...
            theoritical_min_ask_usd_bal = usd[min_ask_ex] - (
                        crypto_per_transaction / (1 - fees[min_ask_ex]['quote'])) * min_ask_price * (
                                                         1 + fees[min_ask_ex]['base'])
            theoritical_max_bid_usd_bal = usd[max_bid_ex] + (
                        crypto_per_transaction / (1 + fees[max_bid_ex]['base']) * (
                            max_bid_price * (1 + fees[max_bid_ex]['quote'])))

            change_usd = (theoritical_min_ask_usd_bal + theoritical_max_bid_usd_bal) - (
            ...
            if max_bid_ex != min_ask_ex and change_usd > float(criteria_usd) and prec_ask_price != min_ask_price and prec_bid_price != max_bid_price:
                ...
                fees_crypto = crypto_per_transaction * (
                            fees[min_ask_ex]['quote']) + crypto_per_transaction * (
                                           fees[max_bid_ex]['base'])
                fees_usd = crypto_per_transaction * max_bid_price * (
                            fees[max_bid_ex]['quote']) + crypto_per_transaction * min_ask_price * (
                                       fees[min_ask_ex]['base'])
                ...
                append_list_file('templates/all_opportunities_profits.txt', change_usd)
                ...
                usd[max_bid_ex] += crypto_per_transaction / (
